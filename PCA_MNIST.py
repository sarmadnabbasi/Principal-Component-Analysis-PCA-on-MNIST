...
print(f"(c) Reconstruction error for the selected sample: {reconstruction_error}")
...
for i in range(X_train.shape[0]):
    x_n = X_centered[i]
    z_n = top_k_eigenvectors.T @ x_n
    x_tilde_n = top_k_eigenvectors @ z_n
    reconstruction_errors.append(np.linalg.norm(x_n - x_tilde_n))
...
for j in range(M_list.shape[0]):
    ...
    for i in range(X_train.shape[0]):
        ...
        if np.any(random_images_idx ==  i):
            ...
            axes[j, int(np.where(random_images_idx == i)[1][0])*2].imshow((x_n[i]+mean).reshape(28, 28), cmap='gray')
            ...
            # Plot reconstructed image
            axes[j, int(np.where(random_images_idx == i)[1][0])*2+1].imshow((x_tilde_n[i]+mean).reshape(28, 28), cmap='gray')
